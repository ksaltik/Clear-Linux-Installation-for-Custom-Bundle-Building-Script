    ...
    echo -e "Then if warning message appears \nabout  - not all space available - ${RED}etc select ( Fix )${NC}"
    echo -e "After Fix there will be info about disk \n${RED}size like - Disk /dev/sda: 67.8GB - "
    echo -e "Then enter resize ( partition number ) \nthe partition number most probably ${RED}2 so"
    echo -e "${RED}Enter resizepart 2 and it will ask you the end? \nenter the above ${RED}value 67.8GB and hit enter and then q to quit"
    sudo parted
